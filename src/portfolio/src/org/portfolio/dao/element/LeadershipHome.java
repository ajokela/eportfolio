/* $Name:  $ */
/* $Id: LeadershipHome.java,v 1.8 2010/10/27 19:24:57 ajokela Exp $ */
/*
 * $Header: /opt/UMN-src/portfolio/src/org/portfolio/dao/element/LeadershipHome.java,v 1.8 2010/10/27 19:24:57 ajokela Exp $
 * $Revision: 1.8 $
...
import org.portfolio.model.Leadership;

/**
 * Handles the persistence of the Leadership data element. <br />
 * 
 * @author John Bush
 * @since 0.1
...
public class LeadershipHome extends AbstractElementHome {
    /**
     * Persists the Leadership Object. Will perform an INSERT if it is a new
     * instance, (i.e., there is no data created), else it will perform an
     * update.
     * 
     * @param data the Leadership object to persist @ if a data type other than
     *            the one expected is found. @ if there are any problems
     *            persisting the data.
     */
    public void store(ElementDataObject data) {
        if (!(data instanceof Leadership)) {
        } else {
            Leadership leadership = (Leadership) data;
            logService.debug("Is instance new?" + leadership.isNew());
            if (leadership.isNew()) {
     ...
     */
    public List<Leadership> findByPersonId(String personId) {
        List<Leadership> elements = new ArrayList<Leadership>();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                leadership.setPersonId(rs.getString("PERSON_ID"));
                leadership.setEntryId(rs.getBigDecimal("ENTRY_ID"));
                leadership.setDateCreated(rs.getTimestamp("DATE_CREATED"));
                leadership.setModifiedDate(rs.getTimestamp("MODIFIED_DATE"));
                leadership.setEntryName(rs.getString("ENTRY_NAME"));
                leadership.setDescription(rs.getString("DESCRIPTION"));
                leadership.setExperience(rs.getString("EXPERIENCE"));
                // logService.debug("Adding instance " + leadership + " to the elements list.");
                elements.add(leadership);
            }
     ...
     * to guarantee that the data object is of type Leadership.
     * 
     * @param leadership the instance to update. @ if there is a problem
     *            updating.
     */
    protected void update(Leadership leadership) {
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = getDataSource().getConnection();
            ...
            ps.setString(1, leadership.getEntryName());
            ps.setString(2, leadership.getDescription());
            ps.setString(3, leadership.getExperience());
            ps.setString(4, leadership.getPersonId());
            ps.setBigDecimal(5, leadership.getEntryId());
     ...
     * guarantee that the data object is of type Leadership.
     * 
     * @param leadership the instance to create. @ if there is a problem
     *            inserting.
     */
    protected void insert(Leadership leadership) {
        Connection conn = null;
        PreparedStatement ps = null;
        int id = (leadership.isRemote()) ? leadership.getEntryId().intValue() : sequenceGenerator.getNextSequenceNumber("entry");
        try {

            conn = getDataSource().getConnection();
            ps = conn.prepareStatement(INSERT_QUERY);
            logService.debug("Preparing insert statement " + INSERT_QUERY);
            ps.setString(1, leadership.getPersonId());
            ps.setInt(2, id);
            leadership.setEntryId(String.valueOf(id));
            ps.setString(3, leadership.getEntryName());
            ps.setString(4, leadership.getDescription());
            ps.setString(5, leadership.getExperience());
            int result = ps.executeUpdate();
     ...
     */
    protected void delete(Leadership leadership) {
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = getDataSource().getConnection();
            ...
            ps.setString(1, leadership.getPersonId());
            ps.setBigDecimal(2, leadership.getEntryId());
            int result = ps.executeUpdate();
    ...
    private final String SELECT_QUERY_ALL = "SELECT " + "PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE" + ",ENTRY_NAME"
            + ",DESCRIPTION" + ",EXPERIENCE" + " FROM LEADERSHIP WHERE PERSON_ID = ?";
    ...
    private final String SELECT_QUERY_SINGLE = "SELECT " + "PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE" + ",ENTRY_NAME"
            + ",DESCRIPTION" + ",EXPERIENCE" + " FROM LEADERSHIP WHERE PERSON_ID = ? AND ENTRY_ID = ?";

    /** The query to select a single instance of NAME for a person. */
    private final String SELECT_QUERY_SINGLE_COUNT = "SELECT COUNT(ENTRY_ID) FROM LEADERSHIP WHERE PERSON_ID = ? AND ENTRY_ID = ?";

    
    /** The query to INSERT a new row into the leadership table. */
    private final String INSERT_QUERY = "INSERT INTO LEADERSHIP" + "(PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE"
            + ",ENTRY_NAME" + ",DESCRIPTION" + ",EXPERIENCE" + ") VALUES (" + "?" + "," + "?" + "," + sysdate + "," + sysdate + "," + "?"
            + "," + "?" + "," + "?" + ")";

    /** The query to INSERT a new row into the leadership table. */
    private final String UPDATE_QUERY = "UPDATE LEADERSHIP SET " + "MODIFIED_DATE = " + sysdate + ",ENTRY_NAME = ?" + ",DESCRIPTION = ?"
            + ",EXPERIENCE = ?" + " WHERE PERSON_ID=? AND ENTRY_ID=?";

    /** The query to DELETE a row from the NAME table. */
    private final String DELETE_QUERY = "DELETE FROM LEADERSHIP " + " WHERE PERSON_ID = ? AND ENTRY_ID = ?";
