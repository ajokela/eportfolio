/* $Name:  $ */
/* $Id: PhoneHome.java,v 1.9 2010/10/27 19:24:57 ajokela Exp $ */
/*
 * $Header: /opt/UMN-src/portfolio/src/org/portfolio/dao/element/PhoneHome.java,v 1.9 2010/10/27 19:24:57 ajokela Exp $
 * $Revision: 1.9 $
...
import org.portfolio.model.Phone;

/**
 * Handles the persistence of the Phone data element. <br />
 * 
 * @author John Bush
 * @since 0.1
...
public class PhoneHome extends AbstractElementHome {
    /**
     * Persists the Phone Object. Will perform an INSERT if it is a new
     * instance, (i.e., there is no data created), else it will perform an
     * update.
     * 
     * @param data the Phone object to persist @ if a data type other than the
    ...
    public void store(ElementDataObject data) {
        if (!(data instanceof Phone)) {
            ...
            logService.debug("Is instance new?" + phone.isNew());
            if (phone.isNew()) {
     ...
     */
    public List<Phone> findByPersonId(String personId) {
        List<Phone> elements = new ArrayList<Phone>();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                phone.setPersonId(rs.getString("PERSON_ID"));
                phone.setEntryId(rs.getBigDecimal("ENTRY_ID"));
                phone.setDateCreated(rs.getTimestamp("DATE_CREATED"));
                phone.setModifiedDate(rs.getTimestamp("MODIFIED_DATE"));
                phone.setEntryName(rs.getString("ENTRY_NAME"));
                phone.setPhone(rs.getString("PHONE"));
                // logService.debug("Adding instance " + phone + " to the elements list.");
                elements.add(phone);
            }
     ...
     * to guarantee that the data object is of type Phone.
     * 
     * @param phone the instance to update. @ if there is a problem updating.
     */
    protected void update(Phone phone) {
        ...
        try {
            ...
            ps.setString(1, phone.getEntryName());
            ps.setString(2, phone.getPhone());
            ps.setString(3, phone.getPersonId());
            ps.setBigDecimal(4, phone.getEntryId());
     ...
     * guarantee that the data object is of type Phone.
     * 
     * @param phone the instance to create. @ if there is a problem inserting.
     */
    protected void insert(Phone phone) {
        Connection conn = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            ps.setString(1, phone.getPersonId());
            ps.setInt(2, id);
            phone.setEntryId(String.valueOf(id));
            ps.setString(3, phone.getEntryName());
     ...
     */
    protected void delete(Phone phone) {
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = getDataSource().getConnection();
            ...
            ps.setString(1, phone.getPersonId());
            ps.setBigDecimal(2, phone.getEntryId());
            int result = ps.executeUpdate();
    ...
    private final String SELECT_QUERY_ALL = "SELECT " + "PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE" + ",ENTRY_NAME"
            + ",PHONE" + " FROM PHONE WHERE PERSON_ID = ?";
    ...
    private final String SELECT_QUERY_SINGLE = "SELECT " + "PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE" + ",ENTRY_NAME"
            + ",PHONE" + " FROM PHONE WHERE PERSON_ID = ? AND ENTRY_ID = ?";

    /** The query to select a single instance of NAME for a person. */
    private final String SELECT_QUERY_SINGLE_COUNT = "SELECT COUNT(ENTRY_ID) FROM PHONE WHERE PERSON_ID = ? AND ENTRY_ID = ?";
    
    /** The query to INSERT a new row into the phone table. */
    private final String INSERT_QUERY = "INSERT INTO PHONE" + "(PERSON_ID" + ",ENTRY_ID" + ",DATE_CREATED" + ",MODIFIED_DATE"
            + ",ENTRY_NAME" + ",PHONE" + ") VALUES (" + "?" + "," + "?" + "," + sysdate + "," + sysdate + "," + "?" + "," + "?" + ")";

    /** The query to INSERT a new row into the phone table. */
    private final String UPDATE_QUERY = "UPDATE PHONE SET " + "MODIFIED_DATE = " + sysdate + ",ENTRY_NAME = ?" + ",PHONE = ?"
            + " WHERE PERSON_ID=? AND ENTRY_ID=?";

    /** The query to DELETE a row from the NAME table. */
    private final String DELETE_QUERY = "DELETE FROM PHONE " + " WHERE PERSON_ID = ? AND ENTRY_ID = ?";
