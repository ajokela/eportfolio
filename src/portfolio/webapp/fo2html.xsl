...
<xsl:stylesheet version="1.0"
	...
	<xsl:output method="html" version="4.0" encoding="utf-8"
		doctype-public="-//W3C//DTD HTML 4.0 Transitional//EN" indent="no" />
	...
	<xsl:template match="fo:root">
		<html>
			...
			<body bgcolor="white" marginwidth="6" marginheight="6"
	...
	<xsl:template match="fo:page-sequence">
		...
		<xsl:variable name="page-master-name">
			<xsl:choose>
				<xsl:when
					test="../fo:layout-master-set/fo:simple-page-master[@master-name=$current-master]">
		...
		<!-- Load the page master into a variable. No problem if it's null; should 
			work the same ;-) -->
		...
		<xsl:apply-templates
			select="fo:static-content[@flow-name = $header-region/@region-name
		...
		<xsl:apply-templates
			select="fo:static-content[@flow-name = $footer-region/@region-name
  ...
  &anchor;
	...
	<xsl:template match="fo:list-item">
		...
		<tr>&add-style;
			<xsl:apply-templates select="fo:list-item-label"
				mode="draw-cell">
	...
	<xsl:template match="fo:list-block/fo:list-item-label">
		...
		<tr>
			...
			<xsl:apply-templates select="following-sibling::fo:list-item-body[1]"
				mode="draw-cell" />
		</tr>
	</xsl:template>
  ...
  &anchor;
	...
	<xsl:template name="enumerate-rowless-cells">
		...
		<xsl:if test="not(@ends-row='true')">
			<xsl:for-each
				select="following-sibling::fo:table-cell[1]
                          [not(@starts-row='true')]">
				<xsl:call-template name="enumerate-rowless-cells" />
			</xsl:for-each>
		</xsl:if>
  ...
  &anchor;
		<img src="{$cleaned-url}">
			<xsl:apply-templates
				select="@height|@width|@*[starts-with(name(),'border')]" />
		</img>
	</xsl:template>
  ...
  &anchor;
	...
	<xsl:template match="fo:retrieve-marker">
		...
		<xsl:choose>
			<xsl:when
				test="@retrieve-position='last-starting-within-page'
                 or @retrieve-position='last-ending-within-page'">
				<xsl:apply-templates select="$matching-markers[position()=last()]"
					mode="retrieve-marker" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="$matching-markers[1]"
					mode="retrieve-marker" />
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="@text-align" mode="collect-style-attributes">
		...
		<xsl:choose>
			<xsl:when test=".='start' or .='inside'">
				left
			</xsl:when>
			<xsl:when test=".='end' or .='outside'">
				right
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="." />
	...
	<xsl:template
		...
		<xsl:variable name="property">
			<xsl:choose>
				<xsl:when test="starts-with(name(), 'border')">
					border
				</xsl:when>
				<xsl:when test="starts-with(name(), 'padding')">
					padding
				</xsl:when>
				<xsl:when test="starts-with(name(), 'margin')">
					margin
				</xsl:when>
				<xsl:when test="starts-with(name(), 'space')">
					margin
				</xsl:when>
				<xsl:when test="contains(name(), '-indent')">
					margin
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="side">
			<xsl:choose>
				<xsl:when test="contains(name(), '-before') or contains(name(), '-top')">
					-top
				</xsl:when>
				<xsl:when
					test="contains(name(), '-after') or contains(name(), '-bottom')">
					-bottom
				</xsl:when>
				<xsl:when
				...
				<xsl:when
					test="contains(name(), '-end') or starts-with(name(), 'end-') or contains(name(), '-right')">
					-right
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="parameter">
			<xsl:choose>
				...
				<xsl:when test="contains(name(), '-style')">
					-style
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template
		match="*[@white-space-collapse='false'
	...
	<xsl:template match="@*" mode="convert-to-pixels">
		<xsl:variable name="scaling-factor">
			<xsl:choose>
				...
				<xsl:when test="contains (., 'em')">
					16
				</xsl:when> <!-- guess: 1em = 12pt -->
				<xsl:otherwise>
					1
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template match="@*" mode="unbracket-url">
		...
		<xsl:choose>
			<xsl:when
				test="(starts-with($href, 'url(') or starts-with($href, 'url ('))
				...
				<xsl:choose>
					<xsl:when
						test="( substring($quoted, 1, 1) = $q and
	...
	<xsl:template match="fo:flow | fo:static-content">
		...
		<div>&add-style;&anchor;
			...
			<xsl:if test=".//fo:footnote">
				<br />
				<hr />
				<xsl:apply-templates select=".//fo:footnote"
					mode="after-text" />
			</xsl:if>
		</div>
	</xsl:template>
	...
	<xsl:template name="add-style-attribute">
		...
		<xsl:if test="string-length($style) &gt; 0">
			<xsl:attribute name="style"><xsl:value-of
				select="normalize-space($style)" /></xsl:attribute>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="@*" mode="get-table-attributes"
		priority="-1" />
	...
	<xsl:template
		match="fo:single-page-master-reference
	...
	<xsl:template match="fo:repeatable-page-master-alternatives">
		<xsl:choose>
			<xsl:when
				test="fo:conditional-page-master-reference[@page-position='first']">
				<xsl:value-of
			...
			<xsl:otherwise> <!-- cannot guess; take the first -->
				<xsl:value-of
					select="fo:conditional-page-master-reference[1]/@master-reference" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="@extent">
		<xsl:attribute name="width"><xsl:apply-templates
			select="." mode="convert-to-pixels" /></xsl:attribute>
	</xsl:template>
