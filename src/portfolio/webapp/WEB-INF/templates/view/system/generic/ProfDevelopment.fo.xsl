...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="ProfDevelopment">
			...
			<fo:table-body>
				<fo:table-row>
					...
					<fo:table-cell xsl:use-attribute-sets="td.elemPresentationCell2">
						<fo:block>
							<!-- xsl:if test="entryName/value"> <fo:inline font-weight="bold">Activity 
								Name: </fo:inline><xsl:value-of select="entryName/value"/><fo:block></fo:block> 
								</xsl:if -->
							<xsl:if test="sponsor/value">
								<fo:inline font-weight="bold">Sponsor:</fo:inline>
								<xsl:value-of select="sponsor/value" />
								<fo:block></fo:block>
							</xsl:if>
							...
							<xsl:if test="certificate/value">
								<fo:inline font-weight="bold">Certificate Earned:</fo:inline>
								<xsl:value-of select="certificate/value" />
								<fo:block></fo:block>
							</xsl:if>
