<%-- This file is not used. See renderHtmlView.jsp instead --%>
    ...
    <c:choose>
      ...
      <c:when test="${item.type == 'entry'}">
		...
		<c:set var="phone" value="Phone Number" scope="request" />
         ...
         <div id="shareEntry${entry.entryKeyId}" style="font-size: 10pt;">
      		<c:if test="${entry.elementName ne email and entry.entryName ne primary and entry.entryName ne preferred and entry.elementName ne phone}">
				<div style="padding-top: 15px; padding-bottom: 15px; font-weight: bold;">
					${entry.entryName}
						<span style="font-weight: normal;">(${entry.elementName})</span>
				</div>
			</c:if>
						...
						<c:if test="${not empty entry.linkAttachments}">
				            ...
				            <ul class="files_urls attachmentsList">
				              <c:forEach var="link" items="${entry.linkAttachments}">
				                <li>
				                  <a href="${fn:escapeXml(link.url)}" target="_blank">${fn:escapeXml(link.entryName)}</a>
				                </li>
				              </c:forEach>
				            </ul>
				            </div>
							...
							<div>
								<ul class="files_urls attachmentsList">
								  <c:forEach var="file" items="${entry.fileAttachments}">
								    <li>
								      <a href="/file/${file.entryKeyId}?portfolioId=${portfolio.shareId}">${fn:escapeXml(file.sampleName)}</a>
								      <c:if test="${not empty file.fileExtension}">(.${file.fileExtension})</c:if>
								    </li>
								  </c:forEach>
								</ul>
							</div>
					              ...
					              <c:forEach var="image" items="${entry.photoAttachments}">
					                <li>
					                  <a href="/photo/${image.entryKeyId}?portfolioId=${portfolio.shareId}&amp;maxWidth=800&amp;maxHeight=800"
					                     title="${fn:escapeXml(image.sampleName)}" target="_blank">
					                    <img src="/photo/${image.entryKeyId}?portfolioId=${portfolio.shareId}&amp;width=75&amp;height=75" alt="" />
					                  </a>
					                  <a href="/photo/${image.entryKeyId}?portfolioId=${portfolio.shareId}&amp;maxWidth=800&amp;maxHeight=800"
					                     title="${fn:escapeXml(image.sampleName)}"
					                     target="_blank" >${fn:escapeXml(image.sampleName)}</a>
					                </li>
					              </c:forEach>
					            </ul>
				            </div>
